...
.. list-table:: Random channel types
   ...
   * - ``sfc64_hash``
     ...
     - Option with the lowest state-initialization overhead when
       channels are frequently reseeded.

The names identify the implementations and the custom seeding used by
``sfc64_hash``. They do not rank speed or label a mode's maturity. In particular,
...
The accelerated modes copy initial states through NumPy's public state
interface, then advance ActivitySim-owned ``uint64`` arrays using compiled PCG64
and SFC64 transitions. They do not scan or modify NumPy's private C state memory.
The accelerated modules are loaded only when an accelerated channel is created;
legacy runs do not initialize them. Regression tests compare outputs and final
states against NumPy, including mixed distribution calls and arithmetic boundaries.
...
its keyed streams.  It uses the `SFC64
