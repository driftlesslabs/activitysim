...
class Settings(PydanticBase, extra="allow", validate_assignment=True):
    ...
    * `test` - Run every relevant calculation using both sharrow and legacy
    ...
    rng_channel_type: Literal["legacy", "pcg64", "sfc64_hash"] = "legacy"
    """
    ...
    * ``"legacy"`` (default)
        ...
        Slower but provided for backward compatibility and reproducibility
        with results generated by older ActivitySim versions.
    * ``"pcg64"``
    ...
    * ``"sfc64_hash"``
        Use :class:`activitysim.core.fast_random.FastChannel`, with a
        vectorised SFC64-based implementation and hash-based
        reseeding.

    The ``"legacy"`` default preserves results from earlier ActivitySim
    versions. New models should generally prefer ``"pcg64"`` for statistically
    robust, higher-throughput streams. Use ``"sfc64_hash"`` only after validating
    that its lower reseeding overhead is appropriate for the model. These names
    identify implementations rather than ranking speed: Numba compilation and robust
    per-row initialization can make ``"pcg64"`` slower than ``"legacy"`` for
